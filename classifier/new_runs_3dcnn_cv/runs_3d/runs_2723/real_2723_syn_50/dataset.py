"""
Dataset for the 3D-CNN cross-validation experiments.
...
The ONLY difference vs. the original 2D classifier dataset is the output
shape: instead of returning a [D, 128, 128] tensor (slices-as-channels for
EfficientNet), we return a [1, D, 128, 128] tensor (a single-channel volume)
so it can be fed directly to the 3D CNN (cnn_ben_3d.MRIClassifier).
"""
...
class MRIDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Stack into a volume and shape it for the 3D CNN: [1, D, H, W]
        image = torch.stack(slices, dim=0)  # [D, 1, 128, 128]
        image = image.squeeze(1)            # [D, 128, 128]
        image = image.unsqueeze(0)          # [1, D, 128, 128]
