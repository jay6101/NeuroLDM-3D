"""
Driver for the 3D-CNN 5-fold CV experiments.

Each variant trains a 3D CNN with 5-fold cross-validation over the combined
val+test pool (fixed train set) and produces, under
    runs/<group>/<variant>/
        parameters.json
...
python run.py --variants real_500 --epochs 1 --n-folds 1 --device cuda:0
...
def build_hyperparams(variant, args):
    hp = copy.deepcopy(config.BASE_HYPERPARAMS)
...
def main():
    parser = argparse.ArgumentParser(description="3D-CNN 5-fold CV runner")
    ...
    parser.add_argument('--runs-dir', default=config.DEFAULT_RUNS_DIR,
    ...
    for v in variants:
        ...
        try:
            summary = train_model(hp, config.MODEL_NAME)
