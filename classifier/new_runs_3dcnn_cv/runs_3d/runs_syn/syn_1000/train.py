...
Training procedure / architecture / hyper-parameters come from
3D_CNN/3D_supervised_HC_vs_TLE (cnn_ben_3d + ReduceLROnPlateau + BCEWithLogits
+ BalancedBatchSampler + steps_per_epoch). The dataset/preprocessing is kept
from the Diffusion classifier (see dataset.py).
"""
...
def train_model(hyperparams, modelname):
    ...
    for fold_idx, (fold_val_df, fold_test_df) in enumerate(folds):
        ...
        val_dataset = MRIDataset(fold_val_df, hyperparams, train=False)
        test_dataset = MRIDataset(fold_test_df, hyperparams, train=False)
        ...
        model = MRIClassifier(
            ...
            filters=hyperparams.get('filters', None),
